...
class RotationConfigForm extends CompatForm
{
    ...
    /** @var ?RotationData */
    protected ?RotationData $rotation = null;
    ...
    public function setRotation(Rotation $rotation): static
    {
        ...
        [$this->rotation, $values] = $this->prepareFormPopulate($rotation);
     ...
     * @return RotationData
     */
    public function getRotation(): RotationData
    {
        // Unlike the usual getter, this is supposed to ignore the respective property

        $members = [];
        if ($this->getValue('members')) {
            foreach (explode(',', $this->getValue('members')) as $memberDef) {
                [$type, $id] = Str::symmetricSplit($memberDef, ':', 2);
                if ($id === null) {
                    continue;
                }
        ...
        return new RotationData(
            $this->rotation?->id,
            $this->scheduleId,
            $this->hasBeenDuplicated() ? 0 : (int) $this->getValue('priority', 0),
            $this->getValue('name', ''),
            $this->getValue('mode'),
    ...
    protected function assembleTwentyFourSevenOptions(FieldsetElement $options): DateTime
    {
        ...
        $firstHandoff = RotationData::parseDateAndTime(
            new DateTimeZone($this->scheduleTimezone),
            $date,
            $at->getValue()
        );
            ...
            'validators' => [
                new CallbackValidator(
                    function ($value, $validator) use ($earliestHandoff, $firstHandoff, $latestHandoff) {
                        $chosenHandoff = RotationData::parseDateAndTime(
                            new DateTimeZone($this->scheduleTimezone),
                            $value,
                            $firstHandoff->format('H:i')
                        );
                        $latestHandoff = RotationData::parseDateAndTime(
                            new DateTimeZone($this->scheduleTimezone),
        ...
        if ($this->getElement('first_handoff')->hasValue()) {
            $this->addHtml(new HtmlElement(
                ...
                DeferredText::create(function () use ($options) {
                    ...
                    $ruleGenerator = $this->getRotation()->yieldRecurrenceRules(1);
    ...
    /**
     * Prepare form population using the given rotation
     *
     * @param Rotation $rotation
     *
     * @return array<RotationData, array>
     */
    private function prepareFormPopulate(Rotation $rotation): array
    {
        $membersForForm = [];
        $membersForData = [];
        foreach ($rotation->member->orderBy('position', SORT_ASC) as $member) {
            if ($member->contact_id !== null) {
                $membersForForm[] = 'contact:' . $member->contact_id;
                $membersForData[] = ['contact', $member->contact_id];
            } else {
                $membersForForm[] = 'group:' . $member->contactgroup_id;
                $membersForData[] = ['contact_group', $member->contactgroup_id];
            }
        }
        ...
        $data = new RotationData(
            ...
            isset($rotation->previousHandoff)
                ? \DateTimeImmutable::createFromMutable($rotation->previousHandoff)
                : null,
            isset($rotation->nextHandoff)
                ? \DateTimeImmutable::createFromMutable($rotation->nextHandoff)
                : null,
            isset($rotation->previousShift)
                ? \DateTimeImmutable::createFromMutable($rotation->previousShift)
                : null
        );
        ...
        $values = [
            ...
            'members' => implode(',', $membersForForm)
        ];
        if (! self::EXPERIMENTAL_OVERRIDES) {
            $values['first_handoff'] = $rotation->first_handoff;
        }
