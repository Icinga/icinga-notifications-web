...
class NotificationHistory extends Model
{
    ...
    {
        return ['event_message', 'object.name'];
    }
    ...
    public static function on(Connection $db): Query
    {
        ...
        $query->on(Query::ON_SELECT_ASSEMBLED, function (Select $select) use ($query) {
            $utilized = $query->getUtilize();
            if (
                isset($utilized['notification_history.object.object_id_tag'])
                || isset($utilized['notification_history.incident.object.object_id_tag'])
            ) {
