...
class EventRuleRenderer implements ItemRenderer
{
    ...
    public function assembleExtendedInfo($item, HtmlDocument $info, string $layout): void
    {
        ...
        if ($rs) {
            ...
            if ($recipientCount) {
                $info->add(new RuleEscalationRecipientBadge(
    ...
    {
        $meta = new HtmlElement('span', new Attributes(['class' => 'meta']));

        if ($item->object_filter) {
            $meta->addHtml(new Icon('filter'));
        }

        $escalationCount = $item->rule_escalation->count();
        if ($escalationCount > 1) {
            $meta->addHtml(new Icon('code-branch'), new Text($escalationCount));
        }
